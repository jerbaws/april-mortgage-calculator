    ...
    for month in range(1, fixed_term_months + 1):
        ...
        if rate != previous_rate or month == 1:
            ...
            if repayment_type == "Repayment":
                payment = balance * (monthly_rate * (1 + monthly_rate) ** total_term_months) / ((1 + monthly_rate) ** total_term_months - 1)
            elif repayment_type == "Interest-Only":
                payment = balance * monthly_rate
            else:
                payment = balance * monthly_rate * 0.8 + (balance * 0.2 * (monthly_rate * (1 + monthly_rate) ** total_term_months) / ((1 + monthly_rate) ** total_term_months - 1))
            previous_rate = rate
            milestones.append({
                "Month": month,
